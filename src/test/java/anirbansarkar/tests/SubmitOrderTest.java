...
import anirbansarkar.pageObjects.RegistrationPage;
 ...
 * - User registration with all required details
 * - Application login with registered credentials
 ...
 * 
 * Test Data: Loaded from PurchaseOrder.json data provider
 * Groups: Purchase (for test categorization)
 * 
...
public class SubmitOrderTest extends BaseTest {
	 ...
	 * 
	 * Test Flow:
	 * 1. User Registration: Register a new user with provided credentials
	 * 2. User Login: Login with registered email and password
	 * 3. Product Selection: Add product to shopping cart
	 * 4. Cart Verification: Verify product is displayed in cart
	 * 5. Checkout: Proceed to payments page
	 * 6. Payment Details: Enter delivery country
	 * 7. Order Placement: Complete the order
	 * 8. Confirmation: Assert order confirmation message
	 * 
	 * @param input HashMap containing test data:
	 *              - firstName: User's first name
	 *              - lastName: User's last name
	 *              - email: User's email address
	 *              - password: User's password
	 *              - mobile: User's phone number
	 *              - occupation: User's occupation (Doctor/Student/Engineer/Scientist)
	 *              - gender: User's gender (Male/Female)
	 *              - order: Product name to order (e.g., "ZARA COAT 3")
	 * 
	 * @throws InterruptedException if thread is interrupted during execution
	 * @throws IOException if there's an issue with file operations or network
	 * 
	...
	public void submitOrder(HashMap<String, String> input) throws InterruptedException, IOException {
		...
		rp.registerUser(
			input.get("firstName"),      // First name of the user
			input.get("lastName"),       // Last name of the user
			input.get("email"),          // Email for registration (also used for login)
			input.get("mobile"),         // Mobile number
			input.get("occupation"),     // Occupation dropdown selection
			input.get("gender"),         // Gender radio button selection
			input.get("password")        // Password for account
		);
		...
		Assert.assertTrue(
			cp.verifyCartProductDisplayed(input.get("order")),
		...
		Assert.assertTrue(
			op.getOrderConfirMsg().equalsIgnoreCase(ordConfirMsg),
			"Order confirmation message mismatch. Expected: " + ordConfirMsg + 
			" but got: " + op.getOrderConfirMsg()
		);
	 ...
	 * 
	 * This test is dependent on submitOrder test completion.
	 * It verifies that the placed order appears in the user's order history.
	 * 
	 * Test Flow:
	 * 1. Login with registered credentials
	 * 2. Navigate to orders/order history page
	 * 3. Verify the placed order is displayed in the history
	 * 
	...
	public void orderHistoryTest(HashMap<String, String> input) throws InterruptedException, IOException {
		...
		Assert.assertTrue(
			op.verifyOrderDisplayed(input.get("order")),
	 ...
	 * 
	 * Loads test data from external JSON file (PurchaseOrder.json).
	 * This enables data-driven testing with multiple test datasets.
	 * 
	 * JSON File Location: 
	 * src/test/java/anirbansarkar/data/PurchaseOrder.json
	 * 
	 ...
	 *     "occupation": "Engineer",
	 *     "gender": "Female",
	 *     "order": "ADIDAS ORIGINAL"
	 *   }
	 * ]
	 * 
	 * @return Object[][] - 2D array of test data
	 *         First test set: mapList.get(0)
